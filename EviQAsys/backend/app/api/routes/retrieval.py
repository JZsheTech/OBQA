...
def test_retrieval(
    *,
    ...
    top_k: int = Query(5, ge=1, le=50, description="Maximum number of candidates to return."),
    doc_id: int | None = Query(None, ge=1, description="Optional document filter inside the collection."),
    elem_types: str | None = Query(
        None,
    ...
    elem_type_list = _parse_elem_types(elem_types)
    try:
...
def _parse_elem_types(raw: str | None) -> list[str] | None:
    if raw is None:
        return None
