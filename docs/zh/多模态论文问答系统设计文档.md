...
graph TD
    ...
    B5 -->|合并文本上下文| C3 -->|带 [Evidence#no] 的答案| B5
    B5 -->|写 turn 与证据映射| C2
...
3. 提问：使用 DSPy 进行文本问句重写；检索服务做向量与元数据查找，记忆服务确保 Evidence 编号连续。
4. 回答：默认仅使用文本上下文（图像元素以 caption 参与）。可选“图像理解”路径：对检索到的 image 元素按 question+query 派生子问题，调用独立的视觉问答接口获取摘要文本，与 caption 合并为文本上下文，再由 DSPy 文本回答模块生成带 `[Evidence#no]` 的答案。完成后写入 `turns` 与 `turn2evidence`，前端用 `bbox/page_no` 高亮。
