...
1. 上传：前端上传 PDF，后端保存元信息，调用 MinerU，标准化 `content_list`，写入 `documents/elements`；其中 `documents.title` 优先采用解析结果中首个 header 元素的标题文本，若不存在有效 header 则回退为文件名。
...
- 图像（`elem_type="image"`）：`text_content` 使用图片 caption（`text_caption`）及同一 section 内上下文；图像像素内容存入 `image_base64`。
- 表格（`elem_type="table"`）：`text_content` 使用表格 caption + markdown 形式的表格正文（来自 MinerU）以及同一 section 内上下文；caption 仍保留在 `text_caption` 字段。
