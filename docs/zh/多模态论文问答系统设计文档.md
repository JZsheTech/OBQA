...
- Chunk 构建规则：
  - 过滤空白/控制字符（`CHUNK_SKIP_PATTERNS`），按 `level_nav` 内顺序聚合纯文本/公式/标题元素；达到 `MIN_CHARACTOR_CHUNK_SIZE` 且元素数 ≥ `MAX_ELEM_CHUNK_SIZE` 或章节结束时落库。
...
- `MIN_CHARACTOR_CHUNK_SIZE` / `MAX_ELEM_CHUNK_SIZE`：chunk builder 的最小字符阈值与合并窗口上限。
