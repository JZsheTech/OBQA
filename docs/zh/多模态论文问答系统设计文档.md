...
graph TD
    ...
    B5 -->|写 turn 与元素映射| C2
    B5 -->|返回| B1 -->|answer([Evidence#no]) + anchors| A
```
...
| 后端 API | `api` 路由 + Pydantic `schemas` | 暴露上传、建 Chat、问答等 HTTP 入口；内部使用 `[Elem#id]`，返回前序列化为 `[Evidence#no]`，并包含 `bbox/page_no`。 |
...
3. 提问：使用 DSPy 进行文本问句重写；检索服务做向量与元数据查找；Evidence 展示编号由 API 层在返回前动态生成。
4. 回答：默认仅使用文本上下文（图像元素以 caption 参与）。可选“图像理解”路径：对检索到的 image 元素按 question+query 派生子问题，调用独立的视觉问答接口获取摘要文本，与 caption 合并为文本上下文，再由 DSPy 文本回答模块生成带 `[Elem#<element_id>]` 的答案；返回前替换为 `[Evidence#no]`。完成后写入 `turns` 与 `turn2element`，前端用 `bbox/page_no` 高亮。
...
- 在同一 chat 维度，按元素“首次出现顺序”动态生成展示编号 `evidence_no`，从 1 开始、跨 turn 累积；不持久化存库。
- 桥表更名为 `turn2element`，主键：`(chat_id, turn_id, element_id)`；保留 `turn_order` 便于查询。
...
- 可选图像理解路径：
  ...
  - 基于原始 `question` 与检索 `query` 生成 1 个针对该图片的派生子问题（文本）；
