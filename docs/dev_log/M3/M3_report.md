...
4. **检索服务与 API**：`services/retrieval/retriever.py` 统一查询入口，新增 `GET /api/retrieval/test` 返回 `{code,data}`；支持 `collection_id` + 可选 `doc_id/elem_types` + `search_mode=hybrid|vector|fulltext`。
...
3. 后端启动后，可通过 `GET /api/retrieval/test?collection_id=<id>&query=...&top_k=5` 与可选 `doc_id / elem_types / search_mode=hybrid|vector|fulltext` 做联调。
