...
- 后端补齐 chat 相关 API（创建、详情含 evidences、turn evidences），默认 LLM 模型切换为 `qwen3:235b`。
...
- **LLM 默认模型**：`LLMSettings.model` 默认改为 `qwen3:235b`，保持其余配置不变。
...
3) 配置：更新 `env_setting.py` 默认 `LLM_MODEL_NAME = qwen3:235b`。
