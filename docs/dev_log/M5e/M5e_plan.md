...
- 后端补齐 chat 相关 API（创建、详情含 evidences、turn evidences），默认 LLM 模型切换为 `x-ai/grok-4.1-fast:free`（OpenRouter，原 `qwen3:235b` 可通过环境变量指定）。
...
- **LLM 默认模型**：`LLMSettings.model` 默认改为 `x-ai/grok-4.1-fast:free`（OpenRouter），保持其余配置不变，仍支持通过环境变量切换回 Ollama。
...
3) 配置：更新 `env_setting.py` 默认 `LLM_MODEL_NAME = x-ai/grok-4.1-fast:free`，`LLM_API_BASE` 默认走 `https://openrouter.ai/api/v1`。
...
- LLM 默认模型已切换为 `x-ai/grok-4.1-fast:free`（OpenRouter）。***
