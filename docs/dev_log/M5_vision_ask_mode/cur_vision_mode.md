...
- 是否接入真实视觉 LLM：是。视觉路径通过 `requests` 直接向可配置的 OpenAI 兼容多模态接口发起 HTTP 调用，默认指向 OpenRouter 的 `x-ai/grok-4-fast`；不存在本地伪造或占位逻辑，只是当接口未部署或元素缺少 `image_base64` 时会降级为仅用 caption/context 的文本描述，不再调用 VLM。
