...
- VQA 调用：`_build_image_evidences` 对每个图片候选用 `ImageQuestionGenerator.generate`（DSPy，有回退）生成派生问句，再调用 `VisionVQAClient.summarize(element_id, derived_question, local_context)`。`summarize` 会查库拿 `image_base64`，拼出包含 question/caption/附近上下文的 prompt，按 OpenAI 兼容格式 POST 到 `VISION_VQA_ENDPOINT`（默认 `OLLAMA_OPENAI_BASE_URL`）与指定 `model`（默认 `qwen2.5-vl-72b`），并从 `choices[0].message.content`/`result` 提取文本。
...
- 是否接入真实视觉 LLM：是。视觉路径通过 `requests` 直接向可配置的 OpenAI 兼容多模态接口发起 HTTP 调用，默认指向本地/网关的 qwen2.5-vl-72b；不存在本地伪造或占位逻辑，只是当接口未部署或元素缺少 `image_base64` 时会降级为仅用 caption/context 的文本描述，不再调用 VLM。
